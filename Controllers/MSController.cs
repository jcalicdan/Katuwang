...
namespace Katuwang.Controllers
{
    public class MSController : Controller
    {
        ...
        public MSController(KatuwangContext db, INotyfService notyfService)
        {
            _db = db;
        ...
        public IActionResult Index()
        {
            ...
            var r401 = _db.R401.Where(x => x.status == "MS" & x.isdeleted == 0);
            ...
            var objList = _db.R401.Where(x => x.status == "MS" & x.isdeleted == 0 & x.year == Convert.ToInt32(cyear) & x.month == cmonth.ToString())
        ...
        public IActionResult Update(MasterlistVM obj)
        {
            if (ModelState.IsValid)
            {
                ...
                var objList = _db.R401.Where(x => x.status == "MS" & x.isdeleted == 0)
                            .Join(_db.Masterlist, x => x.masterlistid, y => y.entryid,
                                ((r401, masterlist) => new R401Masterlist { R401 = r401, Masterlist = masterlist }))
                            .ToList();
        ...
        {
            return await _db.R401.FromSqlRaw("EXEC sp_MS_PreviousMonth {0}, {1}", year, month).ToListAsync();
        }
    }
}
